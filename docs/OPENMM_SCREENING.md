...
- **Minimize has no wall-clock limit:** After Packmol, [`run_openmm_matrix_relax_and_energy`](../src/python/biologix_ai/simulation/openmm_complex.py) calls **`openmm.LocalEnergyMinimizer.minimize(..., maxIterations=BIOLOGIX_AI_OPENMM_MAX_MINIMIZE_STEPS)`** (default **2000**). Each iteration can mean many force evaluations on a **large** system (insulin + **`n_polymers`** chains × oligomer). On **CPU**, one difficult candidate can take **hours**. The **`BIOLOGIX_AI_OPENMM_MATRIX_WALL_CLOCK_S`** limit applies only to the **optional NPT** loop after minimize—not to minimization itself.
