...
- **packmol** (binary on PATH): **required** for MCP **`openmm_evaluate_psmiles`** / **`MDSimulator.evaluate_candidates`**, which run **Packmol matrix encapsulation** (`run_openmm_matrix_relax_and_energy`). Without Packmol, evaluation raises at startup. See `docs/OPENMM_SCREENING.md` for matrix env vars (`INSULIN_AI_OPENMM_MATRIX_*`, etc.).
