...
| TUI frozen mid-run but `docker exec … ps` shows **only** `opencode` + idle MCP (no python/bash child) | OpenCode **snapshot** service failing on every step: `failed to add snapshot files` / `unknown option sparse` in `/root/.local/share/opencode/log/opencode.log`. Fixed in **≥ 0.5.19**: `"snapshot": false` in `.opencode/opencode.jsonc`, conda-forge **git ≥ 2.40**, `OPENCODE_DISABLE_AUTOUPDATE=true`. Until then: `docker kill` + relaunch on 0.5.19. |
| Long discovery runs stall on free OpenCode Zen (`big-pickle`) | Free models can hang mid-stream with no client timeout. For multi-hour autonomous runs, configure a paid provider: `opencode auth login` inside the container, or set `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` in `.env` and pick that model in the TUI. |
...
| `OPENCODE_DISABLE_AUTOUPDATE` | **`true`** | Keeps OpenCode at the Dockerfile-pinned version (no silent upgrade to 1.17.7+) |
