...
- **Stdout rule:** MCP protocol uses stdout only; all logs and OpenMM progress go to stderr (`PYTHONUNBUFFERED=1` in `scripts/run_mcp_server.sh`). Guarded tools redirect stray stdout to stderr.
