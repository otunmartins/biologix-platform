...
**Recommended release:** `ghcr.io/otunmartins/biologix-ai:0.5.9` (tag `v0.5.9`). Pin this tag for reproducible runs; `latest` tracks the same build but can change when a new release ships.
...
docker pull --platform linux/amd64 ghcr.io/otunmartins/biologix-ai:0.5.9

docker run --platform linux/amd64 -it --rm --init \
  -v "$(pwd)/runs:/app/runs" \
  -v "$(pwd)/papers:/app/papers" \
  -v biologix-data:/app/data \
  ghcr.io/otunmartins/biologix-ai:0.5.9
```
