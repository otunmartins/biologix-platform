...
**Golden rule:** If **any** MCP call **times out for any reason**, the session **latches to CLI-only mode** — **all remaining steps** via bash CLI in **`.opencode/MCP_CLI_FALLBACK.md`**; **no further MCP tool calls** in that session.
