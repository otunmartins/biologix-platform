...
**Report figures:** embed `structures/*_complex_chemviz.png` (insulin ribbon + polymer bonded sticks via PyMOL). Do **not** use `*_complex_preview.png` — that file is a matplotlib 3D dot cloud for quick debugging only. Docker image **≥ 0.5.27** ships PyMOL in a separate `pymol-viz` conda env (on PATH in the container).
