...
Run:   uvicorn insulin_ai.http_api.app:app --reload
Docs:  http://localhost:8000/docs
OpenAPI JSON (for Claude Design / codegen): http://localhost:8000/openapi.json

Architecture
------------
All routes call the same insulin_ai.services.* layer as the MCP server.
No science code is duplicated. The REST layer is purely an interface adapter.

CORS
----
Set INSULIN_AI_CORS_ORIGINS env var to a comma-separated list of allowed
...
from insulin_ai.http_api.routers import (
    candidates,
    experiments,
    personas,
    retrosynthesis,
...
_cors_raw = os.environ.get("INSULIN_AI_CORS_ORIGINS", "*")
_origins: List[str] = (
    ["*"] if _cors_raw.strip() == "*"
