...
All outputs go under a single session directory (runs/<id>/ or BIOLOGIX_AI_SESSION_DIR).
"""
...
def _ensure_paths(root: Optional[str] = None) -> str:
    r = root or os.environ.get("BIOLOGIX_AI_ROOT", "")
    if not r:
        here = os.path.abspath(__file__)
        r = os.path.dirname(here)
        for _ in range(3):
            r = os.path.dirname(r)
        if not os.path.isfile(os.path.join(r, "biologix_ai_mcp_server.py")):
            r = os.path.dirname(r)
